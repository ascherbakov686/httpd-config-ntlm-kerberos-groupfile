                                              ...
                                              const void *parsed_require_args)
{
    ...
    while ((w = ap_getword_conf(r->pool, &t)) && w[0]) {
        if (apr_table_get(grpstatus, w)) {
         ...
         array = apr_table_elts(grpstatus);
         if (array->nelts == 1)
         {
            data  = ((struct apr_table_entry_t*)array->elts)->key;
            ap_log_rerror(APLOG_MARK, APLOG_ERR, 0, r,"GroupName %s",data);

        //Own code
        apr_table_add(r->headers_in,"X-Forwarded-Group",data);
        apr_table_add(r->err_headers_out,"X-Forwarded-Group",data);

            return AUTHZ_GRANTED;
        }
        else
        {
            ap_log_rerror(APLOG_MARK, APLOG_ERR, 0, r,"Error: Too many groups for user");
        }
